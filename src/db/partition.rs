...
pub async fn create_future_partitions(pool: &PgPool, config: &Config) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for i in 0..n {
        ...
        let future_str = format!("{}", future.format("%Y-%m-%d"));
        let next_month_str = format!("{}", next_month.format("%Y-%m-%d"));
        ...
        let sql = format!(
            ...
            name, future_str, next_month_str
        );
...
async fn create_daily_ts_partitions(pool: &PgPool, table: &str, today: &NaiveDate, n: u32) {
    for i in 0..n {
        ...
        let name = format!("{}_y{:04}m{:02}d{:02}", table, d.year(), d.month(), d.day());
        let start_str = d.and_time(chrono::NaiveTime::from_hms_opt(0, 0, 0).unwrap()).to_string();
        let end_str = end.and_time(chrono::NaiveTime::from_hms_opt(0, 0, 0).unwrap()).to_string();
        let sql = format!(
            "CREATE TABLE IF NOT EXISTS {} PARTITION OF {} FOR VALUES FROM ('{}') TO ('{}')",
            name, table, start_str, end_str
        );
        create_partition(pool, table, &sql).await;
    }
}
