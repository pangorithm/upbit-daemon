...
pub async fn fetch_and_upsert_markets(
    ...
    for market in &filtered {
        ...
        let (
            ...
            caution_deposit,
            caution_global,
            caution_concentration,
        ) = parse_market_events(market);

        if let Err(e) = upsert_market(
            pool,
            market_code,
            korean_name,
            english_name,
...
fn parse_market_events(market: &Value) -> (bool, bool, bool, bool, bool, bool) {
    ...
    if let Some(events) = market.get("market_event") {
        ...
        let (price_fluctuations, trading_volume, deposit_amount, global_price, concentration) =
            match caution_obj {
                Some(ref c) => (
    ...
    let price_fluctuations = market["market_event_caution_price_fluctuations"]
        .as_bool()
        .unwrap_or(false);
    let trading_volume = market["market_event_caution_trading_volume_soloing"]
        .as_bool()
        .unwrap_or(false);
    let deposit_amount = market["market_event_caution_deposit_amount_soloing"]
        .as_bool()
        .unwrap_or(false);
    let global_price = market["market_event_caution_global_price_differences"]
        .as_bool()
        .unwrap_or(false);
    let concentration = market["market_event_caution_concentration_of_small_accounts"]
        .as_bool()
        .unwrap_or(false);
    (
        warning,
